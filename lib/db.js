...
for (const col of [
  ...
  // Formato da DM: 'text' (link no meio do texto) ou 'button' (botão clicável).
  "dm_kind TEXT DEFAULT 'text'",
  "dm_btn_label TEXT DEFAULT ''",
  "dm_btn_url TEXT DEFAULT ''",
]) {
  try {
...
export function createRule({ platform, keyword, match_type = "contains", reply_public = "", dm_text = "", active = 1, strategy = "", step_label = "", media_id = "", replies_json = "", once_per_user = 0, dm_kind = "text", dm_btn_label = "", dm_btn_url = "" } = {}) {
  try {
    const now = Math.floor(Date.now() / 1000);
    const info = db
      .prepare(
        `INSERT INTO automation_rules(platform, keyword, match_type, reply_public, dm_text, active, strategy, step_label, media_id, replies_json, once_per_user, dm_kind, dm_btn_label, dm_btn_url, created, updated)
         VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`
      )
      .run(platform, keyword, match_type, reply_public || "", dm_text || "", active ? 1 : 0, String(strategy || "").slice(0, 40), String(step_label || "").slice(0, 60), String(media_id || ""), String(replies_json || ""), once_per_user ? 1 : 0, dm_kind === "button" ? "button" : "text", String(dm_btn_label || "").slice(0, 20), String(dm_btn_url || ""), now, now);
...
export function updateRule(id, fields = {}) {
  const allowed = ["platform", "keyword", "match_type", "reply_public", "dm_text", "active", "strategy", "step_label", "media_id", "replies_json", "once_per_user", "dm_kind", "dm_btn_label", "dm_btn_url"];
...
/** Este comentario ja foi tratado alguma vez? (usado ao recuperar antigos) */
export function jaTratadoComentario(platform, commentId) {
  try {
    return Boolean(
      db.prepare("SELECT 1 FROM automation_events WHERE id = ?").get(`${platform}:${commentId}`)
    );
  } catch {
    return false;
  }
...
export function automationStats({ since = 0 } = {}) {
  try {
    const startOfDay = new Date();
    startOfDay.setHours(0, 0, 0, 0);
    const todaySec = Math.floor(startOfDay.getTime() / 1000);
    // Periodo escolhido no painel (hoje / 7 dias / 30 dias / desde sempre).
    const desde = Number(since) || 0;
    const filtro = desde ? "WHERE created >= ?" : "";
    const p = desde ? [desde] : [];
    const r = db
      .prepare(
    ...
    const perStrat = db
      .prepare(
        `SELECT COALESCE(NULLIF(strategy,''),'Geral') AS strategy,
          ...
          WHERE link_token IS NOT NULL ${desde ? "AND created >= ?" : ""}
    ...
    // Mesmas contas, mas separadas por rede (Instagram vs Facebook).
    const perPlat = db
      .prepare(
        `SELECT platform,
                COUNT(*) AS total,
                ...
                SUM(CASE WHEN purchased_at IS NOT NULL THEN 1 ELSE 0 END) AS purchases,
                COALESCE(SUM(purchase_amount), 0) AS revenue
           FROM automation_events ${filtro} GROUP BY platform`
      )
      .all(todaySec, ...p);
    const byPlatform = {};
    for (const p of perPlat) {
    ...
    // Últimos 14 dias, por dia e por rede — alimenta o gráfico do painel.
    const since14 = todaySec - 13 * 86400;
    const serieRows = db
      .prepare(
        `SELECT platform, CAST((created - ?) / 86400 AS INTEGER) AS dia, COUNT(*) AS n
           FROM automation_events WHERE created >= ? GROUP BY 1, 2`
      )
      .all(since14, since14);
    ...
    for (const row of serieRows) {
      const slot = serie[row.dia];
      if (slot && (row.platform === "ig" || row.platform === "fb")) slot[row.platform] = row.n || 0;
    }
