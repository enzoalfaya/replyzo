 ...
 */
export function claimAutomation(platform, commentId, ruleId, { username = "" } = {}) {
  try {
    const info = db
      .prepare(
        `INSERT OR IGNORE INTO automation_events(id, platform, comment_id, rule_id, username, created)
         VALUES(?,?,?,?,?,?)`
      )
      .run(
        `${platform}:${commentId}`,
...
export function automationStats() {
  try {
    ...
    return { total: 0, dms: 0, publics: 0, errors: 0, today: 0, withLink: 0, clicked: 0, cta: 0, sales: 0, purchases: 0, revenue: 0 };
  }
}
