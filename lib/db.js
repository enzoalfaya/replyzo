...
for (const col of ["strategy TEXT DEFAULT ''", "step_label TEXT DEFAULT ''"]) {
  try {
...
export function createRule({ platform, keyword, match_type = "contains", reply_public = "", dm_text = "", active = 1, strategy = "", step_label = "" } = {}) {
  try {
    const now = Math.floor(Date.now() / 1000);
    const info = db
      .prepare(
        `INSERT INTO automation_rules(platform, keyword, match_type, reply_public, dm_text, active, strategy, step_label, created, updated)
         VALUES(?,?,?,?,?,?,?,?,?,?)`
      )
      .run(platform, keyword, match_type, reply_public || "", dm_text || "", active ? 1 : 0, String(strategy || "").slice(0, 40), String(step_label || "").slice(0, 60), now, now);
...
export function updateRule(id, fields = {}) {
  const allowed = ["platform", "keyword", "match_type", "reply_public", "dm_text", "active", "strategy", "step_label"];
  const sets = [];
  const params = [];
