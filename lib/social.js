...
async function handleInstagramComment(value = {}) {
  ...
  if (rule.reply_public) {
    const r = await igReplyToComment(commentId, applyTemplate(rule.reply_public, who));
    if (r.ok) didPublic = 1;
    else { ok = 0; errors.push("public: " + r.error); }
  }
  if (rule.dm_text) {
    const r = await igPrivateReply(commentId, applyTemplate(rule.dm_text, who));
    if (r.ok) didDm = 1;
...
async function handleFacebookFeed(value = {}) {
  ...
  // No Facebook vem o nome completo; usamos so o primeiro nome ("João Silva" -> "João").
  const who = (value.from?.name || "").trim().split(/\s+/)[0] || "";
  ...
  if (rule.reply_public) {
    const r = await fbReplyToComment(commentId, applyTemplate(rule.reply_public, who));
    if (r.ok) didPublic = 1;
