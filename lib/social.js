...
const IG_POLL_MEDIA = Number(process.env.IG_POLL_MEDIA) || 2; // recentes, sempre
const IG_POLL_ROTATE = Number(process.env.IG_POLL_ROTATE) || 3; // antigas, a vez
...
const IG_POLL_MAX = Number(process.env.IG_POLL_MAX) || 12;
...
export async function pollInstagramComments() {
  ...
  try {
    ...
    if (comRegra.length) {
      ...
      for (let i = 0; i < Math.min(IG_POLL_ROTATE, comRegra.length); i++) {
  ...
  return { ok: true, seen, handled, limiteAtingido };
}
