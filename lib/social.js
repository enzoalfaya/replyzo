...
// -----------------------------------------------------------------------------
//  Token do Instagram com AUTO-RENOVAÇÃO.
//  O token IG dura 60 dias; renovamo-lo sozinhos para nunca expirar. O token
//  vivo fica na BD (sobrevive a reinícios); o IG_ACCESS_TOKEN do .env é a
//  "semente" — se o trocares manualmente no Render, detetamos e recomeçamos.
// -----------------------------------------------------------------------------
let igToken = null;
function getIgToken() {
  if (igToken !== null) return igToken;
  const seed = getSetting("ig_seed");
  if (IG_ACCESS_TOKEN && IG_ACCESS_TOKEN !== seed) {
    // Semente nova (1º arranque ou troca manual no Render) → recomeça.
    setSetting("ig_seed", IG_ACCESS_TOKEN);
    setSetting("ig_token", IG_ACCESS_TOKEN);
    setSetting("ig_refreshed_at", String(Math.floor(Date.now() / 1000)));
    igToken = IG_ACCESS_TOKEN;
  } else {
    igToken = getSetting("ig_token") || IG_ACCESS_TOKEN || "";
  }
  return igToken;
}

/**
 * Renova o token IG (+60 dias). Só o faz se já passaram >2 dias desde a última
 * renovação (evita chamadas a mais quando o servidor reinicia com frequência).
 */
export async function refreshIgToken({ force = false } = {}) {
  const tok = getIgToken();
  if (!tok) return { ok: false, error: "sem token IG" };
  const last = Number(getSetting("ig_refreshed_at")) || 0;
  const ageDays = (Math.floor(Date.now() / 1000) - last) / 86400;
  if (!force && ageDays < 2) return { ok: true, skipped: true, ageDays: Math.round(ageDays) };
  try {
    ...
    setSetting("ig_token", igToken);
    setSetting("ig_refreshed_at", String(Math.floor(Date.now() / 1000)));
    return { ok: true, expiresInDays: Math.round((data.expires_in || 0) / 86400) };
  } catch (err) {
...
export function igConfigured() {
  return Boolean(IG_USER_ID && getIgToken());
}
...
export async function diagnostics() {
  ...
  if (out.ig.configured) {
    try {
      const r = await fetch(`${IG_GRAPH}/me?fields=username&access_token=${encodeURIComponent(getIgToken())}`);
...
/** Instagram: resposta publica a um comentario (passa pelo limitador). */
export function igReplyToComment(commentId, message) {
  ...
  return rateLimited(() => graphPost(IG_GRAPH, `${commentId}/replies`, { message }, getIgToken()));
}
 ...
 * (Não passa pelo limitador — é setup, não uma resposta.)
 */
export function igSubscribeApp() {
  if (!igConfigured()) return Promise.resolve({ ok: false, error: "IG nao configurado" });
  return graphPost(IG_GRAPH, "me/subscribed_apps", { subscribed_fields: "comments" }, getIgToken());
}
